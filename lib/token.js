      ...
      */
      try {
        if (windowEl &&
          ...
          resolve(urlParamsToObject(windowEl.location.hash));
...
function getToken(sdk, options) {
  ...
  .then(function(oauthParams) {
    ...
    urls = getOAuthUrls(sdk, oauthParams);
    ...
    switch (flowType) {
      ...
      case 'POPUP':
        ...
        if (oauthParams.responseMode === 'fragment') {
          var windowOrigin = getOrigin(window.location.href);
...
function getWithRedirect(sdk, options) {
  ...
  return prepareOauthParams(sdk, options)
    .then(function(oauthParams) {
      ...
      cookies.set(REDIRECT_NONCE_COOKIE_NAME, oauthParams.nonce, null, sdk.options.cookies);

      // Set state cookie for servers to validate state
      cookies.set(REDIRECT_STATE_COOKIE_NAME, oauthParams.state, null, sdk.options.cookies);
